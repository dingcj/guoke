...
HI_S32 SAMPLE_COMM_REGION_AttachToChn(HI_S32 HandleNum,RGN_TYPE_E enType, MPP_CHN_S *pstMppChn)
{
    ...
    for(i =MinHadle; i <MinHadle+HandleNum;i++)
    {
        ...
        {
            stChnAttr.unChnAttr.stCoverExChn.stRect.s32X = 20 + 100*(i - CoverExMinHandle);
...
HI_S32 SAMPLE_COMM_REGION_AttachToChn_2BPP(HI_S32 HandleNum,RGN_TYPE_E enType, MPP_CHN_S *pstMppChn)
{
    ...
    stChnAttr.unChnAttr.stOverlayChn.stInvertColor.enChgMod = LESSTHAN_LUM_THRESH;
    stChnAttr.unChnAttr.stOverlayChn.stInvertColor.bInvColEn = HI_FALSE;
    stChnAttr.unChnAttr.stOverlayChn.u16ColorLUT[0] = 0x7FE0;//yellow
    stChnAttr.unChnAttr.stOverlayChn.u16ColorLUT[1] = 0x0;
    stChnAttr.unChnAttr.stOverlayChn.enAttachDest = ATTACH_JPEG_MAIN;
    ...
    /*attach to Chn*/
    s32Ret = SAMPLE_REGION_AttachToChn(MinHadle,pstMppChn,&stChnAttr);
    if(HI_SUCCESS !=s32Ret)
    {
        SAMPLE_PRT("SAMPLE_REGION_AttachToChn failed!\n");
        return HI_FAILURE;
...
HI_S32 SAMPLE_COMM_REGION_2BPP_DrawLine(RGN_HANDLE Handle)
{
    ...
    stBitmap.u32Width  = ALIGN_DOWN(stVencChnAttr.stVencAttr.u32PicWidth, 4);
    stBitmap.u32Height = ALIGN_DOWN(stVencChnAttr.stVencAttr.u32PicHeight, 4);
    stBitmap.enPixelFormat = PIXEL_FORMAT_ARGB_2BPP;
    u32OsdSize = (stBitmap.u32Width/4)*stBitmap.u32Height;
    stBitmap.pData = malloc(u32OsdSize);
    if(NULL == stBitmap.pData)
    {
        SAMPLE_PRT("malloc osd memroy err!\n");
        return HI_FAILURE;
    }
    memset(stBitmap.pData, 0, u32OsdSize);

    stSize.u32Width  = stBitmap.u32Width;
    stSize.u32Height = stBitmap.u32Height;

    /* Draw lines */
    astLine[0].stPoint1.s32X = 300;
    astLine[0].stPoint1.s32Y = 100;
    astLine[0].stPoint2.s32X = 1720;
    astLine[0].stPoint2.s32Y = 100;
    astLine[1].stPoint1.s32X = 300;
    astLine[1].stPoint1.s32Y = 100;
    astLine[1].stPoint2.s32X = 100;
    astLine[1].stPoint2.s32Y = 980;
    astLine[2].stPoint1.s32X = 100;
    astLine[2].stPoint1.s32Y = 980;
    astLine[2].stPoint2.s32X = 1520;
    astLine[2].stPoint2.s32Y = 980;
    astLine[3].stPoint1.s32X = 1720;
    astLine[3].stPoint1.s32Y = 100;
    astLine[3].stPoint2.s32X = 1520;
    astLine[3].stPoint2.s32Y = 980;
    for(i=0; i<u32Num; i++){
        astLine[i].u32Thick = 2;
        astLine[i].bDisplay = HI_TRUE;
        s32Ret = SAMPLE_2BPP_DrawLine((HI_U8*)stBitmap.pData, &stSize, &astLine[i]);
        if(HI_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("SAMPLE_COMM_2BPP_DrawLine failed!\n");
            return HI_FAILURE;
        }
    }
    ...
    stChn.s32DevId = 0;
    s32Ret = HI_MPI_RGN_GetDisplayAttr(Handle, &stChn, &stRgnChnAttr);
    if(HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("HI_MPI_RGN_SetBitMap failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    }
    stRgnChnAttr.unChnAttr.stOverlayChn.u16ColorLUT[0] = 0x03E0;//green
    s32Ret = HI_MPI_RGN_SetDisplayAttr(Handle, &stChn, &stRgnChnAttr);
    if(HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("HI_MPI_RGN_SetBitMap failed with %#x!\n", s32Ret);
        return HI_FAILURE;
    ...
    /* Clear lines,  set bDisplay to HI_FALSE. */
    for(i=0; i<u32Num; i++){
        astLine[i].bDisplay = HI_FALSE;
        s32Ret = SAMPLE_2BPP_DrawLine((HI_U8*)stBitmap.pData, &stSize, &astLine[i]);
        if(HI_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("SAMPLE_COMM_2BPP_DrawLine failed!\n");
    ...
    /* Draw lines */
    astLine[0].stPoint1.s32X = 960;
    astLine[0].stPoint1.s32Y = 200;
    astLine[0].stPoint2.s32X = 200;
    astLine[0].stPoint2.s32Y = 540;
    astLine[1].stPoint1.s32X = 200;
    astLine[1].stPoint1.s32Y = 540;
    astLine[1].stPoint2.s32X = 960;
    astLine[1].stPoint2.s32Y = 880;
    astLine[2].stPoint1.s32X = 960;
    astLine[2].stPoint1.s32Y = 200;
    astLine[2].stPoint2.s32X = 1720;
    astLine[2].stPoint2.s32Y = 540;
    astLine[3].stPoint1.s32X = 1720;
    astLine[3].stPoint1.s32Y = 540;
    astLine[3].stPoint2.s32X = 960;
    astLine[3].stPoint2.s32Y = 880;
    for(i=0; i<u32Num; i++){
        astLine[i].u32Thick = 2;
        astLine[i].bDisplay = HI_TRUE;
        s32Ret = SAMPLE_2BPP_DrawLine((HI_U8*)stBitmap.pData, &stSize, &astLine[i]);
